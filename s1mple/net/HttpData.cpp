...
//  sprintf(strfile, "files%s", "/index.html");
    if (pos < 0) {
        fileName_ = "../s1mple/net/files/index.html";
        HTTPVersion_ = HTTP_11;
...
AnalysisState HttpData::analysisRequest() {
    if (method_ == METHOD_POST) {
        ...
        size_t position = inBuffer_.find('=',0);
        if (position < 0)
            return ANALYSIS_ERROR;
        string temp = solveSudoku(inBuffer_.substr(position+1));
        separate<9,'<'>(temp);
        separate<10,'b'>(temp);
        separate<11,'r'>(temp);
        separate<12,'>'>(temp);
        ...
        body_buff += "<HTML><TITLE>Index</TITLE><BODY><H1>Welcome to S1mple Server.</H1><p>Let`s do a Sudoku!</p>";
        body_buff += "<p>The answer is :</p>";
        body_buff += temp;
        body_buff += "</BODY></HTML>";
